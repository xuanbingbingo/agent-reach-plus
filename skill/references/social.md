...
小红书、抖音、Twitter/X、B站、V2EX、Reddit。
...
## 抖音 / Douyin（OpenCLI，需桌面 Chrome + 登录）

```bash
# 搜索视频（返回 desc/author/url/点赞等）
opencli douyin search "query" -f json

# 某用户作品列表（含互动数据 + play_url 直链）
opencli douyin user-videos SEC_UID -f json

# 话题热点词 / 作品数据 / 地理位置
opencli douyin hashtag ACTION -f yaml
opencli douyin stats AWEME_ID -f yaml
```
